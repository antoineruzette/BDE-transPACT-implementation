...
class GenericModulesTest(unittest2.TestCase):
    def setUp(self):
        ...
        self.options = Namespace(full_hmmer=True, clusterblast=True,
                                 ...
                                 smcogs=True, cassis=True, tta=True, run_asf=True)
        ...
        mock('antismash.generic_modules.cassis.check_prereqs',
             returns=['cassis'], tracker=self.tt)
        mock('antismash.generic_modules.tta.check_prereqs',
             returns=['tta'], tracker=self.tt)
        mock('antismash.generic_modules.active_site_finder.check_prereqs',
             returns=['asf'], tracker=self.tt)
    ...
    Called antismash.generic_modules.active_site_finder.check_prereqs(Namespace(...))
    Called antismash.generic_modules.tta.check_prereqs(Namespace(...))
    ...
    Called antismash.generic_modules.cassis.check_prereqs(Namespace(...))"""
        expected = ['fullhmmer', 'genefinding', 'hmm_detection', 'asf', 'tta',
