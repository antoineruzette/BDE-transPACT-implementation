...
import pysvg.gradient # avoid LinearGradient collision/redefinition
from pysvg.structure import Svg, G, Defs
from pysvg.text import Text
from pysvg.builders import StyleBuilder, ShapeBuilder, Stop

def create_svgs(options, seq_record):
    logging.debug("Writing visualization SVGs")
    screenwidth = 1024
    seq_record.geneposdict = create_genecluster_svg(options, seq_record, screenwidth)
    options.asdb_accs = load_asdb_accs()
...
def load_asdb_accs():
    '''Load list of accession numbers available in the antiSMASH DB

    Currently, this is from a file containing one accession per line
    '''
    # one of the wildcard imports is overriding "path"
    from os import path
    with open(path.join(path.dirname(__file__), 'accessions_in_db.txt'), 'r') as fh:
        text = fh.read()

    text_accs = text.split('\n')
    if text_accs[-1] == '':
        text_accs = text_accs[:-1]
    # one of the wildcard imports is overriding "set"
    import __builtin__
    asdb_accs = __builtin__.set(text_accs)
...
def LinearGradient(startcolor,stopcolor,gradientname):
    d = Defs()
    lg = pysvg.gradient.LinearGradient()
...
def clusterblastresults(options, queryclusternumber,hitclusternumbers,queryclusterdata,colorschemedict,rgbcolorscheme, screenwidth, file_id, idx):
    ...
    largestclustersize, _, qclustersize = findclustersizes(options, hitclusternumbers, hdata, qstarts, qends)
    ...
    s = clusterblast_gene_arrows(s, qnrgenes, qdata, hdata, rgbcolorscheme, queryclusternumber, hitclusternumbers, hitclusterdata, screenwidth, file_id, idx, strandsbalancedict, options.asdb_accs)
...
def clusterblast_gene_arrows(s, qnrgenes, qdata, hdata, rgbcolorscheme, queryclusternumber, hitclusternumbers, hitclusterdata, screenwidth, file_id, idx, strandsbalancedict, asdb_accs):
    queryclustergenes = hitclusterdata[1][3]
    queryclustergenesdetails = hitclusterdata[1][4]
    ...
    for x in range(qnrgenes):
        ...
        start, end, _, desc = queryclustergenesdetails[queryclustergenes[x]]
    ...
    for m in hitclusternumbers:
        ...
        hitcluster_acc, cluster_number = hitclusterdata[m][6].rsplit('_', 1)
        hitcluster_desc = hitclusterdata[m][5][m-1].split('\t')[-1].replace('_', ' ')
        group = G()
        if hitcluster_acc.startswith('BGC'):
            acc = Text('<a xlink:href="http://mibig.secondarymetabolites.org/repository/' + hitcluster_acc + '/index.html#cluster-1" target="_blank">'
                       + hitcluster_acc + '</a>: ' + hitcluster_desc, 5, 20 + 50 * (hitclusternumbers.index(m) + 1))
        elif hitcluster_acc in asdb_accs:
            acc = Text('<a xlink:href="http://antismash-db.secondarymetabolites.org/output/' + hitcluster_acc + '/index.html#cluster-' + cluster_number[1:] + '" target="_blank">'
                       + hitcluster_acc + '_' + cluster_number + '</a>: ' + hitcluster_desc, 5, 20 + 50 * (hitclusternumbers.index(m) + 1))
        else:
            acc = Text(hitcluster_acc + '_' + cluster_number + ": " + hitcluster_desc, 5, 20 + 50 * (hitclusternumbers.index(m) + 1))
        acc.set_class("clusterblast-acc")
        group.addElement(acc)
        ...
        for x in range(hnrgenes):
            ...
            start, end, _, desc, _ = hitclustergenesdetails[hitclustergenes[x]]
