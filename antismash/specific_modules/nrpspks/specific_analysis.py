...
from nrpspks_classification import classify_nrpspks_domains_ks
from nrps_utils import resolve_predicat_domain_specificity
...
def specific_analysis(seq_record, options):
    ...
    transatpks = list(set(utils.get_cluster_features_of_type(seq_record, "transatpks")))
    ...
    if len(pksnrpsvars.pksnrpscoregenes) > 0:
        ...
        if options.transatpks_da and len(transatpks) > 0:
            utils.log_status("Aligning Tans-AT PKS domains")
