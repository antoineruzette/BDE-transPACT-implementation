...
import antismash.generic_modules.clusterblast.clusterblast as clusterblast
import antismash.utils as utils
...
class TestBlastParsing(unittest.TestCase):
    def setUp(self):
        ...
        mock('utils.get_gene_acc', returns=None)
        ...
        mock('clusterblast.fastaseqlengths', returns={})
    ...
    def parse_subject_wrapper(self, subject_line):
        ...
        seqlengths = {}
        return clusterblast.parse_subject(subject_line, seqlengths,
                                          geneclustergenes, seq_record)

    def read_sample_data(self, filename="diamond_output_sample.txt"):
    ...
    def test_unique_pairings_filter(self):
        ...
        sample = clusterblast.uniqueblasthitfilter(data)
        self.assertEqual(len(sample), len(data))
        self.assertEqual(sample, clusterblast.uniqueblasthitfilter(data*2))

        # test empty
        data = [[],["a"],["abc"]]
        results = clusterblast.uniqueblasthitfilter(data)
    ...
    def test_blastparse(self):
        queries, clusters = clusterblast.blastparse(self.sample_data, 0, 0, None)
        ...
        queries, clusters = clusterblast.blastparse(self.sample_data, coverage_threshold, 0, None)
        ...
        ident_threshold = 35
        queries, clusters = clusterblast.blastparse(self.sample_data, 0, ident_threshold, None)
        ...
        queries, clusters = clusterblast.blastparse(self.sample_data, coverage_threshold, ident_threshold, None)
    ...
    def test_blastparse_on_empty(self):
        for blast in ["", "\n", "\r\n", "\n\n"]:
            queries, clusters = clusterblast.blastparse(blast, 0, 0, None)
    ...
    def test_parse_all_single_cluster(self):
        ...
        def parse_all_wrapper(coverage_threshold, ident_threshold):
            clusters_by_number, queries_by_number = clusterblast.parse_all_clusters(self.sample_data,
                                    coverage_threshold, ident_threshold, None)
            # make sure we only found one cluster number
            self.assertEqual(len(clusters_by_number), 1)
            self.assertEqual(clusters_by_number.keys(), [24])
            self.assertEqual(len(queries_by_number), 1)
            self.assertEqual(queries_by_number.keys(), [24])
    ...
    def test_parse_all_multi_cluster(self):
        ...
        sample_data = self.read_sample_data("diamond_output_sample_multicluster.txt")
        clusters_by_number, queries_by_number = clusterblast.parse_all_clusters(sample_data, 0, 0, None)
    ...
    def test_parse_all_empty(self):
        for sample_data in ["", "\n", "\r\n", "\n\n"]:
            clusters, queries = clusterblast.parse_all_clusters(sample_data, 0, 0, None)
        self.assertEqual(len(clusters), 0)
        self.assertEqual(len(queries), 0)
...
class TestSubject(unittest.TestCase):
    def test_init(self):
        ...
        s = clusterblast.Subject(*expected)
...
class TestQuery(unittest.TestCase):
    def test_init(self):
        ...
        q = clusterblast.Query(query_line, 0)
    ...
    def test_subject_tracking(self):
        q = clusterblast.Query("input|c1|0-759|-|CAG25751.1|putative", 0)
        s1 = clusterblast.Subject("a1", "a", 1, 2, "+", "c", 0.5, 1, 0.5, 1e-8, "loc")
        q.add_subject(s1)
        ...
        self.assertEqual(q.subjects.keys(), ["a1"])
        s2 = clusterblast.Subject("a2", "b", 1, 2, "+", "c", 0.5, 1, 0.5, 1e-8, "loc")
        q.add_subject(s2)
        for container in containers:
        ...
        s3 = clusterblast.Subject("a3", "a", 1, 2, "+", "c", 0.5, 1, 0.5, 1e-8, "loc")
        q.add_subject(s3)
        ...
        self.assertEqual(q.subjects.keys(), ["a1", "a2", "a3"])
...
class TestSubjectParsing(unittest.TestCase):
    def setUp(self):
        ...
        mock('clusterblast.fastaseqlengths', returns=self.seqlengths)

    def tearDown(self):
        restore()

    def parse_subject_wrapper(self, subject_line):
        return clusterblast.parse_subject(subject_line, self.seqlengths,
    ...
    def test_invalid_input(self):
        ...
        with self.assertRaises(ValueError) as context: #value since
            self.parse_subject_wrapper(subject_line)
        #start missing end
