...
def annotate_geneclusters(seq_record, options):
    ...
    for cf_cluster in cf_clusters:
        ...
        for cluster in cluster_features:
            ...
            features_in_cluster = utils.get_cluster_cds_features(cluster, seq_record)
            cluster_sig_genes = [gene for gene in secmet_cds_features if gene in features_in_cluster]
                ...
                if ((cluster.location.end + cluster.location.start) / 2) in cf_cluster.location:
                    ...
                    for sig_gene in cluster_sig_genes:
                        ...
                        if cf_cluster.location.start > startpoint:
                            cf_cluster.location = FeatureLocation(startpoint, cf_cluster.location.end)
                        if cf_cluster.location.end < endpoint:
                            cf_cluster.location = FeatureLocation(cf_cluster.location.start, endpoint)
                    cluster_border = SeqFeature(cf_cluster.location, type="cluster_border")
        ...
        if not overlaps and not ('borderpredict_only' in options and options.borderpredict_only):
