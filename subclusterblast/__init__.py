...
from ..clusterblast.clusterblast import internal_homology_blast, load_clusterblast_database
from ..clusterblast.data_loading import prepare_data, generate_Storage_for_cb
...
def run_subclusterblast(seq_record, options):
    ...
    clusters, proteins = load_clusterblast_database(seq_record, searchtype="subclusters")
    if not options.clusterblast:
        seq_record.internalhomologygroupsdict = internal_homology_blast(seq_record)
    perform_subclusterblast(options, seq_record, clusters, proteins)
